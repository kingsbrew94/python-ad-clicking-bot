...
class View {
    ...
    {
        if((is_string($response) || is_numeric($response) || $response === null) && is_empty(self::$controller_payload)) {
            echo $response;
    ...
    static public function csrf_token_valid()
    {
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if(!$token_valid)
               throw new \Exception(json_encode(new Dto($state=false,$message='Invalid request: CSRF token not specified',$payload=null)));
        }
        return $token_valid;
    }
